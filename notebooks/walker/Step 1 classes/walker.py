...
class Walker:
    ...
    def __init__(self, sigma_i, sigma_j, size, map_type='flat'):
        ...
        self.context_map = self.create_context_map(size, map_type)
    ...
    def sample_next_step(self, current_i, current_j, random_state=np.random):
        ...
        # probability map
        next_step_map = self.next_step_proposal(current_i, current_j, self.sigma_i, self.sigma_j,
                                        self.size)
        next_step_probability = self.compute_next_step_probability(next_step_map, self.context_map)
    ...
    def create_context_map(self, size, map_type):
    ...
    def next_step_proposal(self, current_i, current_j, sigma_i, sigma_j, size):
    ...
    def compute_next_step_probability(self, next_step_map, context_map):
